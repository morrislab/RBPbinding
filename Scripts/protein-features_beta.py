...
if '--fasta' in sys.argv:
    sequencefile = sys.argv[sys.argv.index("--fasta")+1]
...
elif '--gapcharacter' in sys.argv:
    gapcharacter = sys.argv[sys.argv.index('--gapcharacter')+1]
    if ',' in gapcharacter:
        gapcharacter = gapcharacter.split(',')
        for gapc in gapcharacter:
            aa.append(gapc)
    else:
        aa.append(gapcharacter)
...
# introduce N to Blosum: with N == 0 
    ...
    if '--Residuesmoothing' in sys.argv:
        ...
        elif '--posmismatch' in sys.argv:
            pskgap = True
            gapsize = int(sys.argv[sys.argv.index('--posmismatch')+1])
            kmerfeat = []
            for kend in range(start-gapsize,k+1-gapsize):    
                kmers = ['']
                for i in range(kend):
        ...
        if '--seqcountweight' in sys.argv:
            def unique(carr):
                return np.unique(carr, return_counts = True)
        else:
            def unique(carr):
                uni = np.unique(carr)
                return uni, np.ones(len(uni))
            ...
            for s, seq in enumerate(seqarr):
                for kend in range(start,k+1):
                    ...
                    for d in range(len(domkmer)):
                        kat, katnum = unique(domkmer[d])
                        #if len(kat) > 1:
                            #katmask = np.isin(kat, kmers) ## too slow
                            #kat = kat[katmask]
                            #delar = []
                        ...
                        if blosum:
                            ...
                            osco = orthscore(len(kat))
                            for ie, ks in enumerate(kat):
                                addkmer(ks, kcountvec, kmers, katnum[ie]*osco, kmasks)
                        else:
                            for ie, ks in enumerate(kat):
                                kcountvec[kmers == ks] += orthscore(len(kat)) * katnum[ie]
            return kcountvec, iy
        ...
        for i, seqarr in enumerate(sequence):
            for s, seq in enumerate(seqarr):
                ...
                #print kmers[np.where(featmat[:,i] == 1)]



if '--removegapcounts' in sys.argv:
    mask = np.zeros(len(kmers))
    for gapc in gapcharacter:
        for kc, kmer in enumerate(kmers):
            if gapc in kmer:
                mask[kc] = 1
    mask = mask == 0
    print 'with gaps', len(kmers)
    kmers = kmers[mask]
    print 'without gaps', len(kmers)
    featmat = featmat[mask]
