...
proteinnames = np.genfromtxt(sys.argv[2], dtype = str)
motifs = np.genfromtxt(sys.argv[3], dtype = str)
domains = np.genfromtxt(sys.argv[4], dtype = str)

xthreshold = float(sys.argv[5])
ythreshold = float(sys.argv[6])

uniquemotifs = np.unique(data[:,1])
...
for u, unmot  in enumerate(uniquemotifs):
    ...
    title = proteinnames[ploc, 3] +' ('+ proteinnames[ploc, 4][0]+proteinnames[ploc, 4].split('_')[-1][0]+'); '+motifs[list(motifs[:, 0]).index(unmot), 1]+'; '+domains[list(domains[:,1]).index(unmot), 2]
