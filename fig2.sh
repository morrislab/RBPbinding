...
jplepredpdb=${Imask}'Rncmpt.aaseq.ext15_domain_fusedjplesvd0.95_svdsignificant_response0.95_maplsq_-testset_PreconstructionZ-norm_kmer2sequenceRncmpt_templates-id50-msim3.5_all.txt'
conspredpdb=${Imask}'Conservation_RRMpdb_Eval0.001-ext15_RRMpdb.rrm1-domain_homologs_maxsim0.95_minhom0.3Rncmpt_templates-id50-msim3.5_all.txt'
rfpredpdb=${Imask}'RF/rf_complete_wz5sum-Nonexnorm-testinterfaceRncmpt_templates-id50-msim3.5_all.txt'
...
python Scripts/kmer_AUC.2.py ${Imask}Rncmpt_templates-id50.0-msim3.5_all.txt $jplepredpdb $conspredpdb JPLE,Conservation --boxroc --boxpr --diffscatter --combine --savefig AUC_pdb_jple_vs_conservation.jpg
# Figure 2D, structure plot
pymol ${Imask}RNCMPT00121_to_4ed5_B_CRncmpt.aaseq.ext15_domain_fusedjplesvd0.95hom_svdsignificant_response0.95_maplsq_decglobal_-testset_PreconstructionZ-norm_kmer2sequence_split.pdb ${Imask}showpdb.pml



##### Supplementary Figures Figure2
# Plot k-mer overlap between KH and RRM
python Scripts/venn_kmeroverlap.py ${indir}{masterfile} ${outdir}Rncmpt.aaseq.ext15_domain_fused_5pm1_5mer_features.npz RRM,KH

# Estimate noise in specificty measurements and determine optimal number of eigenvectors
python Scripts/Eigenvalue.py ${indir}${zscores} Rncmpt.aaseq.ext15_domain_fused_5pm1_5mer_features.npz $rrmkhlist 0.95 ${outdir}
...
python Scripts/scatter_predictions.py --files ${outdir}Performance/DeepL/lrbert_set_complete__LR0.0001fiTrue-testset_profile_pcorrelation.dat ${outdir}Performance/JPLE/jplesvd122_set_complete__svdsignificant_response122.0_maplsq_declocal_-testset_profile_pcorrelation.dat --definecolums -2 -2 --minlength 354 --setnames UnirepDLfeatures JPLE  --plotlim 0,1.025 --label 'Pearson' --savefig ${supfig}Scatter_Pearson_JPLE-bert.jpg
...
python Scripts/kmer_AUC.2.py ${Imask}Rncmpt_templates-id50.0-msim3.5_all.txt $jplepredpdb $rfpredpdb JPLE,Conservation --boxroc --boxpr --diffscatter --combine --savefig ${supfig}AUC_pdb_jple_vs_conservation.jpg
